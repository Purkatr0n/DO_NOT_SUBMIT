...
/**
 * This class uk.ac.aber.cs21120.rhymes.tests the rhyming parts of the assignment: Pronunciation.rhymesWith() first,
...
public class RhymeTests {
    ...
    //
    // The first part of this class uk.ac.aber.cs21120.rhymes.tests Pronunciation.rhymesWith.
    // Because it works with Pronunciation objects, it's quite tedious to set up the uk.ac.aber.cs21120.rhymes.tests.
    // The later Dictionary uk.ac.aber.cs21120.rhymes.tests are simpler to write because they work with words, and so they
    ...
    //
    // The second part of this file uk.ac.aber.cs21120.rhymes.tests Dictionary.getRhymes. A lot of this needs to load the dictionary, so we do
    // that on demand into a static member if it's not loaded already. That means the previous uk.ac.aber.cs21120.rhymes.tests should still run.
    //
    ...
    private static IDictionary getDictionary(){
        if(dictionary == null){
            ...
            if(dictionary.getWordCount() == 0){
                dictionary = null; // so we get this exception for subsequent uk.ac.aber.cs21120.rhymes.tests
    ...
    /**
     * This uk.ac.aber.cs21120.rhymes.tests only one valid rhyme of "ejects", so should rhyme with fewer things
     */
    @Test
    @Order(150)
